...
public class IconRepositoryCustomImpl extends AbstractRepository implements IconRepositoryCustom {
    ...
    @Override
    public DataTable<IconEntity> findByCriteria(HashMap<String, Object> criteria, String[] orderBy, int pageIndex, int pageSize) {
        String selectSql = "SELECT w FROM IconEntity w";
        Query query = entityManager.createQuery(selectSql + buildCriteria(criteria, orderBy), IconEntity.class);
        for (Map.Entry<String, Object> entry : criteria.entrySet()) {
            query.setParameter(entry.getKey(), entry.getValue());
        }

        DataTable<IconEntity> dataTable = new DataTable<>();
        ...
        String countSql = "SELECT COUNT(w) FROM IconEntity w";
