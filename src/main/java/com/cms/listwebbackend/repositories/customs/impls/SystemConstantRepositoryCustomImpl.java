...
public class SystemConstantRepositoryCustomImpl extends AbstractRepository implements SystemConstantRepositoryCustom {
    ...
    @Override
    public DataTable<SystemConstantEntity> findByCriteria(HashMap<String, Object> criteria, String[] orderBy, int pageIndex, int pageSize) {
        String selectSql = "SELECT w FROM SystemConstantEntity w";
        Query query = entityManager.createQuery(selectSql + buildCriteria(criteria, orderBy), SystemConstantEntity.class);
        for (Map.Entry<String, Object> entry : criteria.entrySet()) {
            query.setParameter(entry.getKey(), entry.getValue());
        }

        DataTable<SystemConstantEntity> dataTable = new DataTable<>();
        ...
        String countSql = "SELECT COUNT(w) FROM SystemConstantEntity w";
